...
Use `.agent-bridge/` as the coordination source of truth when collaborating with Claude, helper agents, or humans. Treat it as a planning room, not only an inbox.
...
- `.agent-bridge/inbox/codex/` for questions, answers, holds, reviews, handoffs, and delegation from Claude.
...
```bash
agent-bridge presence update --agent codex --task TASK-ID --status working --files src/file.ts
```
